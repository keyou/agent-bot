...
斜杠命令支持唯一前缀：`/mo`、`/mod`、`/mode` 均打开 `/model` 模型设置卡片，参数按匹配到的命令校验。前缀存在歧义时需补充更多字符，未知命令不会作为 Prompt 发给 Agent。

修改飞书群名会同步更新群主会话当前绑定的任务标题，删除自动生成的前缀或使用其他格式也可以。能够识别配置格式或匹配当前 Agent 的前缀时仍提取任务标题，否则使用完整的新群名，并沿用标题的空白和长度规范化规则。改群名不会切换任务 Agent、修改话题或历史任务，也不影响新建群的命名格式；空名称、没有当前任务或当前任务已关闭时仍跳过。
...
只有显式开启 `/mute` 或 `/mute on` 的群，普通 @ 提问才会自动补充近期成员文字和图片。话题内仅查询当前话题，普通群主会话排除其他话题消息。最多倒序读取 2 页、每页 30 条，从中选取本次 @ 之前最多 20 条消息；文字上限 16,000 字符、单条 2,000 字符，额外图片最多 6 张，优先最近内容。排除机器人、已删除及后续消息，不自动展开文件附件或合并转发。卡片 Prompt 保留原始提问，补充内容标记为参考资料而非操作授权。提交成功后按任务、远端分支和群/话题持久化有限的去重记录。从新到旧扫描，遇到第一条已提交给当前 Agent 会话的消息（普通提问或已补充的上下文）立即停止：不重复附带该消息，不提取更早内容，也不再请求更早一页。仅在队列中尚未提交的上下文只去重，不作为停止边界。`/mute off`、私聊、未 @ 消息、斜杠及 Shell 命令不触发补读；原有话题根消息和显式引用逻辑不变。机器人需要相应的飞书历史消息、群消息和资源权限；读取或图片下载失败会明确提示，不向 Agent 提交残缺请求，也不会自动开启权限。
...
`/fork` 和 `/forkgroup` 会从已完成的工作创建分支，不会中断正在执行的轮次。`/sessions` 用于跨项目、跨 Provider 管理任务，每页最多显示 10 个任务；切换 Provider 不会让任务从列表或搜索结果中消失。项目菜单提供 `New` 和 `NewGroup`。每个任务的 `SwitchGroup` 会创建新群并绑定同一个任务，保留历史、标题和运行设置，不创建新 Agent 任务，也不 fork，原会话绑定不变。若任务正在 Agent Bot 中执行，本轮回复仍留在原会话，新群发来的消息先排队；正在外部客户端执行的任务需要等待完成后再切换。展开任务后会直接显示最后一个用户 Prompt 的前 50 个字符、更新时间以及任务级操作。运行中任务还会显示独立的 `Stop` 按钮，包括当前任务、其他会话和外部客户端中的任务；点击发送正常的 Agent 中断信号，不切换任务、不结束进程。列表刷新保留搜索条件和页码，并根据 Agent 的实际状态保留或移除 Stop；空闲后才提供 Archive。点击 `Turns` 可查看对应任务的轮次，包括尚未绑定到 Agent Bot 的任务，不会切换当前任务；非当前任务仅供查看，需要先切换任务才能 Reset。`/turns` 用于恢复对话上下文，不会回退本地文件。

发送 `/update` 可同时检查最新正式版和 Alpha 版，查看更新简介后，点击 **Update <版本号>** 选择更新。更新简介优先显示对应版本的中文内容（包括随包提供的已发布版本译文），没有中文时回退原文。只有配置的机器人所有者可以执行安装；仅检查不会安装。已安装或低于当前版本的版本不提供更新按钮。下载校验在后台进行，随后等待任务和最终回复完成，再安全重启；失败时沿用现有回滚保护。关闭自动检查后仍可使用此命令；源码或 `npm link` 安装只能查看版本，不能原地更新。选择过期或下载失败后，请重新发送 `/update`。
...
`/status` 状态卡片底部也提供 **Preview**，与 Refresh、Stop/Switch 放在同一行。点击可查看正在执行的轮次，空闲时则查看最近一轮；刷新卡片后会更新目标。对于尚未被 Agent Bot 记录的外部任务，打开 Preview 时会从 Agent 按需读取所选轮次并缓存完整执行详情；读取失败可刷新页面重试。没有轮次、无法读取历史且无本地记录，或文件预览服务不可用时不显示按钮。查看其他任务的状态不会切换或接管任务。

思考卡片底部操作区提供 `Preview` 按钮，可在浏览器中查看本轮已保存的执行时间线，采用紧凑布局并适配手机，思考块直接以内容标题作为折叠标题，展开后显示内容。工具块只有一级折叠，默认只显示有意义的单行命令摘要、简短耗时（如 `58s` 或 `2:32`）和完成状态。展开后先显示以 `$` 开头的完整命令，随后直接显示结果、图片和修改文件。相同的输出和错误日志只显示一次。执行中的 Turn 会通过局部更新实时显示变化，保留手动展开状态、滚动位置和未变化的图片节点，避免图片反复加载；Token 等元数据更新不会替换时间线。折叠工具块和文件变更汇总首屏只加载摘要，展开时才请求命令、日志、图片和文件列表；再次展开复用已加载内容，运行中只刷新已展开且内容变化的详情，加载失败可点击重试。已结束的 Turn 不再建立 SSE 实时连接。“正在等待 Agent 返回进度”只在尚无可显示进度时出现，已有回答、其他可见进展或 Turn 已结束时不再显示。点击文件变更列表或工具文件列表中的文件，可在新标签页预览其当前内容；已删除或不可用的文件保留为路径文字。Turn Preview 与本地文件链接共用相同的签名、访问地址和网络范围。“最终回答”旁提供下载、复制两个图标，导出原始 Markdown（保留格式、代码块和原始链接），而非渲染后的网页内容。下载为 UTF-8 `.md` 文件；复制支持 HTTP/局域网页面的回退方式，并显示成功或失败提示。已结束页面及实时到达的最终回答均可使用。

Turn Preview 完整保留已记录的执行内容，不再反复把整轮内容写入数据库。页面按增量实时更新，折叠工具展开时加载完整详情。备份时请保留整个 Profile 数据目录，以同时保存执行记录和任务信息；已有历史仍可查看。
...
Turn Preview 和 Markdown 文件预览中的 Mermaid 图表支持「预览 / 源码」切换，默认显示预览。使用 `mermaid` 代码块和 `flowchart` 或 `graph` 语法即可，也支持 `flowchart` 代码块。实时更新会保留手动选择的模式；语法错误或尚未完整的图表会回退到源码。图表在本地浏览器中渲染，不上传外部服务；图表默认按可用宽度等比缩小，不会放大小图。手机端预览高度最多约半屏，长图可纵向滑动；点击「原大 / 适应」可查看细节或恢复适配，实时更新会保留该选择。
...
命令授权卡片使用简短的“命令执行确认”标题，优先显示申请原因，命令放入默认折叠的区域，不再把整条命令重复显示为加粗标题。摘要省略可识别的 Shell 包装，展开后仍保留原始包装。命令或说明超长时明确提示截断，请在 Turn Preview 查看完整原始命令后再授权。四个授权操作及其权限范围不变；Preview 保持只读，实时更新保留命令展开状态和滚动位置。
...
Turn Preview 顶部在处理和工具执行期间统一显示“正在处理”；工具详情和其他状态提示保持不变。

Header 不再显示上下文压缩统计，“上下文”Token 数放在“模型调用”前面（没有模型调用次数时，放在工具数量前面），使用 Agent 在本轮最近一次上报的上下文大小，不是累计消耗或模型容量。数值局部实时更新，压缩后收到较小值时同步下降；悬停可查看精确值。历史轮次显示当时保存的值，未上报或数值无效时隐藏，不从其他 Token 数推算，也不额外读取历史。

Turn Preview 先显示每轮保存的 Provider，再显示模型，不添加可见的 Provider 前缀。切换设置只影响后续轮次，不改变当前轮次或历史轮次的标记；旧快照未记录 Provider 时不显示，也不会用任务当前设置补填。

Header 还显示本轮“模型调用 N 次”，按有效 Token 用量更新次数统计：首次正用量计一次，之后累计用量每次增长计一次，优先采用含缓存的总用量，缺失时采用非缓存用量。重复、回退、无效及仅上下文变化的通知不增加次数。数值局部实时更新，重启后保留；不修改飞书思考卡片。已有 Preview 事件在正常回放时可还原次数，缺少完整记录的旧快照或未上报用量的 Agent 则不显示；不额外读取 Agent 历史或扫描 rollout。

Agent 提供数据时，Turn Preview 在原有非缓存 Token 数量旁显示本轮总计（输入加输出，包含缓存命中），不再单独显示缓存命中 Token 数；底层统计保持不变。数值实时更新，使用简写单位，悬停可查看精确值；重复用量通知不会重复累计。缺失的明细和旧快照不作推算，也不会读取任务历史补填；飞书卡片的 Token 统计口径保持不变。
...
CSV 文件（`.csv`，不区分大小写）默认显示只读表格，带行号、列字母和固定标签，可横向及纵向滚动；首行保留为数据，不自动当作表头。支持标准逗号分隔、引号内逗号和换行、双引号转义、空单元格，以及现有 UTF-8、带 BOM 的 UTF-16、GB18030 编码识别；数字字符串和公式均按普通文本显示。预览最多读取开头 2 MiB，表格最多显示 500 条记录、50 列、每格 4000 个字符，超限或引号格式错误时明确提示，字节边界处不完整的记录不展示。代码视图最多显示 2000 个物理行，原始文件和下载保持完整内容；实时更新保留所选模式和表格滚动位置。

Provider、模型、思考强度和权限设置会作用于当前任务，同时保存到对应 Agent 的 `defaults` 中。以后创建没有同 Agent 设置可继承的新任务时，会使用这些默认值；每个已配置 Agent 分别保存自己的设置。

App Server 任务切换权限时，下一轮会同时应用审批与沙箱策略：`auto` 为关闭沙箱且不弹审批，`confirm` 为工作区可写沙箱并按需审批。可在 `/permissions` 卡片中选择，或使用 `agentbot task permissions auto|confirm`。切换无需重启 Agent、Fork 或新建任务，不改变已在执行的工具进程，也不会通过追加消息（steer）修改当前轮次沙箱。工作区额外可写目录、网络访问及临时目录排除项保留 Agent 的配置。
...
异常崩溃后，不再仅凭 Codex 日志缺少结束事件就把任务判为运行中：未加载的任务还需要确认有存活的写入进程。Agent Bot 确认会话恢复成功后才创建新的运行卡片；写锁冲突会停止自动恢复并显示占用卡片，其他尚未确认恢复成功的故障每 5 秒重试，在 1 分钟重试窗口耗尽后标记中断并通知用户，不会自动关闭外部进程。已经确认仍在执行的轮次继续监控，临时断连也不会被该恢复超时误判为空闲。
