...
`/status` 可为尚无本地快照的外部轮次生成签名 Preview 链接，只保存已有的 `turn_runtime_origins` 定位信息，不创建任务、群绑定、快照或最终回复投递记录。首次签名页面 GET 从原 Agent 读取所选轮次，使用摘要分页和该轮完整记录；HEAD、详情及 SSE 请求不主动触发远端读取。并发读取合并，成功后的完整详情仅缓存到 Preview 日志，后续复用。任务元数据读取失败不丢弃执行详情，不以任务当前模型配置标记历史轮次。读取期间若出现真实 Agent Bot 快照，以该快照为准。失败页面支持刷新重试，不改变任务状态；未知轮次仍返回 404。这条路径不会接管外部正在执行的轮次。
...
`/update` 并发查询 npm 的 `latest` 与 `alpha`，限制响应大小和超时；两个渠道独立失败，简介读取失败时显示对应 GitHub tag 的日志链接。只允许更新到更高的正式版或 Alpha 精确版本。更新选择绑定卡片消息和所有者，30 分钟过期；准备更新前一次性消费该卡片的所有选择。拒绝重复、转发、过期和非所有者操作。手动更新与每日自动更新共享准备阶段互斥，在隐藏子进程中执行 npm 准备，激活复用安全重启与回滚。准备计划保存发起会话/话题通知目标，以便重启恢复。`updates.enabled` 仅控制每日检查，不控制此命令。
