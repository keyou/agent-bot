...
The same server provides signed Turn Preview URLs for progress cards. A preview renders the persisted `TurnViewState` chronological activity stream rather than rescanning Codex rollout files, so it does not resume or acquire a writer for the App Server thread. The page exposes only the Prompt, plans, Commentary/reasoning summaries, tool commands and complete retained results, file summaries, errors, and final response already available to Agent Bot. Commands start expanded and outputs collapsed; identical output/error logs are merged after normalizing line endings and trimming outer whitespace. Shell wrappers use the same cleanup as progress cards. REPL arguments are rendered as readable code when a Provider supplies an `input` or code field. Tool start times and durations are shown when recorded; only running tools tick live. It polls the SQLite snapshot only while a page has an active SSE connection, sends complete idempotent snapshots when state changes, preserves both expanded and collapsed disclosures and tool-code scroll positions, and lets `EventSource` reconnect across transient network failures or Worker restarts. Terminal previews stop their client stream after the final snapshot. The URL is a Profile-specific bearer credential governed by the same `fileViewer.host` and `publicBaseUrl` reachability as file links.
