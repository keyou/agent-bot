...
The same server provides signed Turn Preview URLs. Complete execution records live beside SQLite in `turn-previews/<first two hash digits>/<SHA256(turnId)>.jsonl`, isolated by Profile. The two-digit hash buckets are created on demand (at most 256 directories); all reads and writes use this single layout. This is a presentation journal, never an agent/tool replay mechanism. Records are appended in 250 ms / 256 KiB batches; terminal states and safe shutdown flush, with fsync at critical boundaries. SQLite retains a bounded 80-activity card projection, routing/recovery metadata, Prompt, final response and the delivery ledger; ordinary writes coalesce over 750 ms. Full outputs/reasoning and unbounded timelines are excluded from new snapshots. Cumulative tool/progress/reasoning strings are fingerprinted and encoded as append or replacement records, avoiding duplicate streamed plus final output.

Preview reads byte-cursor batches and SSE sends changed activity summaries only. No viewer reads or rendering run when no page is open. Details load in full on expansion; up to four detail projections below 16 MiB are cached with a 60-second idle lifetime. Plain command output streams transfer suffixes; replacements and structured REPL results fall back to a full per-tool detail response. Terminal pages stop SSE; reconnects resume using Last-Event-ID. Incomplete final JSONL lines are ignored during reads and removed before appending after restart; malformed complete records or write failures are reported, not silently discarded. Legacy snapshots remain readable without a startup-wide migration or VACUUM. Existing database/WAL sizes do not automatically shrink. Back up SQLite and the journal directory together. Signed URLs remain Profile-specific bearer credentials governed by `fileViewer` reachability.

`/status` can generate a signed Preview link for an external Turn with no local snapshot. It records only the existing `turn_runtime_origins` locator; no session, chat binding, snapshot, or final-delivery record is created. The first signed page GET reads the selected Turn from its original runtime, using summary pagination plus that Turn’s full record; HEAD, detail and SSE requests do not initiate remote reads. Concurrent reads coalesce. Successful full details are stored only in the Preview journal; subsequent views reuse it. Metadata failures do not discard execution details, and current task model settings are not used to label historical execution. An Agent Bot snapshot arriving during the read takes precedence. Read failures return a retryable page without changing task state; unknown Turn IDs remain 404. This does not attach to externally running Turns.

Turn Preview uses the existing highlight.js dependency to highlight explicitly labelled Markdown fences server-side. It includes the common language set plus PowerShell, Dockerfile, and Windows batch, with `zsh` and `pwsh` aliases. Unknown languages, unlabelled blocks, parser failures, and blocks exceeding 64 KiB of UTF-8 source fall back to escaped, untruncated code. A 32-entry/1 MiB LRU cache avoids re-highlighting unchanged code on SSE snapshots. Colors are scoped to Markdown code blocks and follow the browser light/dark preference; Mermaid rendering and tool logs are unchanged.

Input attachments are stored as optional `promptImagePaths` on `TurnViewState` and `localImagePaths` on user activities, separate from bounded display text. Pending-Turn promotion, steering/reconciliation, queued starts, and snapshot recovery retain these paths. Preview resolves them through the existing signed file service with `raw=1`; missing files get a placeholder. The additive JSON fields require no database migration, and historical user activities without paths are not backfilled.

Turn Preview recognizes `mermaid` and `flowchart` fences in Prompt, Commentary, and answer Markdown. Markdown file previews reuse the same renderer, controls, sizing rules, and security policy; full-file Code view and raw/download content remain unchanged. File updates retain matching diagram nodes, modes, sizes, and scroll positions, re-rendering only changed sources. The browser lazy-loads the pinned Mermaid distribution from the same-origin `/assets/mermaid.js` endpoint (respecting `fileViewer.publicBaseUrl` prefixes), not a CDN. Strict security mode disables HTML labels and interactive callbacks; frontmatter/config directives and sources over 50,000 characters are rejected, and graphs are capped at 500 edges. Rendered SVG has active elements and external links removed; the existing viewer CSP remains unchanged. Each block defaults to Preview with width fitting, keeps its selected mode, fit/actual-size choice, and unchanged SVG across SSE updates, and shows escaped source when loading or rendering fails. SVG width is capped by both its viewBox width and its container width, preserving aspect ratio and avoiding enlargement. Narrow/touch screens cap the preview at 480 px or 55 svh (55 vh fallback); long diagrams scroll vertically. Actual size restores the viewBox width with block-local scrolling, while Fit resets the block scroll position. CSS sizing adapts to orientation changes without re-rendering Mermaid. Ordinary code blocks and tool output remain unchanged.

HTML text files (`.html` and `.htm`, case-insensitive) have preview/code modes. The preview iframe loads the signed file URL with `render=html`, streams the complete original document in its detected encoding, and carries a file-version query for live updates. Both the iframe and the response CSP use `sandbox allow-scripts` without `allow-same-origin`; the response sandbox also applies when the rendered URL is opened directly. Inline scripts/styles and embedded data/blob media are allowed, but external/local resource loading, network requests, forms, popups, and top-level navigation are not enabled. The outer viewer retains its own restrictive script policy, and raw/download responses are unchanged. Source remains escaped, syntax-highlighted where possible, limited to 2 MiB, and addressable with `#L<n>`.
...
`/status` obtains an exact App Server Turn count by paging `thread/turns/list` with `itemsView: "summary"`; it never loads full historical tool output for this metric. Codex rollout disk usage comes from a filesystem metadata lookup on the unstable `Thread.path` returned by App Server. A failure in either metric falls back to locally known Turns or an unknown disk size without failing the status command. Loaded Codex sessions cache the exact count after a status read and track newly started Turns. Context-compaction activities reuse that cache, fall back to Agent Bot's local task graph, and stat the rollout path directly, so compaction handling does not issue a history read.
...
Reasoning Preview receives `item/reasoning/summaryTextDelta`, `item/reasoning/textDelta`, and authoritative `item/completed` reasoning arrays. Optional `TurnViewState.reasoningItems` stores complete summary/content sections separately, anchored to existing activity IDs; legacy `reasoning:*:<summaryIndex>` activities remain a summary fallback. Only summary deltas continue through the existing card progress path. Content and completion updates persist without scheduling card refreshes. Preview extracts the first explicit leading Markdown heading or standalone bold title into the disclosure header, removes only that title from the expanded text, and renders all supplied summary/content sections without field labels. Linked/image headings retain their original body markup to preserve navigation and attachments; untitled text remains intact. Heading detection parses only a candidate first line, not the full Markdown. Preview details use `reasoning:<itemId>` keys and revision-aware lazy loading. No database schema migration or remote history backfill is performed.
...
Every `turn/start` also carries both `approvalPolicy` and `sandboxPolicy`: `auto` maps to `never` + `dangerFullAccess`, and `confirm` to `on-request` + `workspaceWrite`. Switching modes takes effect on the next Turn without restarting the shared App Server or resuming/forking the thread; `turn/steer` does not carry these overrides. Workspace policy options come from the thread lifecycle response; if unavailable, one cwd-scoped `config/read` resolves them lazily and the runtime caches them per session. This preserves configured writable roots, network access, and temporary-directory exclusions without reading history. Invalid/unavailable configuration blocks the new Turn; policy rejection is not retried without sandbox restrictions.

`/provider`, `/model`, `/thinking`, and `/permissions` open one Card 2.0 execution-settings surface with the matching tab active. All four commands reject arguments; tab navigation and setting changes use card callbacks only. Provider choices come from App Server `config/read`; for Codex, Agent Bot also keeps the implicit built-in `openai` Provider available when it is absent from the explicit `model_providers` map. The built-in OpenAI Provider uses App Server `model/list`. A configured custom Provider uses `GET <base_url>/models` when available, including its `query_params`, `http_headers`, `env_http_headers`, and configured Bearer credential; credentials remain internal and are never included in cards or model records. A Provider change keeps the previous model when it appears in the target catalog, otherwise selects the marked default, then the first returned model. Matching App Server catalog metadata supplies reasoning options when a custom endpoint returns only model IDs. If model discovery is unavailable, the current or configured model becomes a single fallback candidate so discovery does not block switching; Codex still verifies the effective Provider and model before Agent Bot persists the result. Every successful Provider, model, reasoning, or permission change updates the current task, atomically writes the complete effective setting group to that Agent's defaults in `config.yaml`, refreshes the same card in place, and applies from the next request. The equivalent CLI task-setting commands use the same persistence path.
...
On every Worker startup, Agent Bot scans nonterminal turn attempts before releasing persisted Prompt queues. Only attempts with activity during the preceding ten minutes are eligible; runtime events and a one-minute heartbeat refresh the durable activity timestamp while a turn remains active. Older unfinished attempts are marked interrupted without a recovery notification or continuation. Eligible recovery first notifies the original private chat, group, or topic. A remotely completed turn is synchronized and only its pending final delivery is resumed. A remotely active turn is reattached to a new progress card and polled until terminal. A stale App Server turn or an interrupted ACP process is continued in the same task and workspace with a new turn and a new progress card; the continuation Prompt tells the Agent to inspect existing effects before repeating work. Writer conflicts end automatic recovery and display the existing occupancy controls without closing any process. Other unconfirmed recovery failures retry every five seconds within a one-minute window, then close the recovery card and mark the attempt interrupted. Failed activity checks do not authorize replaying the Prompt. A confirmed live turn continues to be monitored through temporary connection outages; the retry window is not a turn execution timeout.
...
Agent Bot also reconciles when turn identifiers change, terminal notifications arrive, or control requests fail. Stale persisted `inProgress` turns are not considered active unless the App Server reports active, or an unloaded thread has both an unfinished rollout and an OS-reported live writer. Process-inspection errors remain errors rather than being treated as idle. Writer checks are cached for five seconds independently of the incremental rollout cache; loaded idle/completed state takes precedence over old rollout starts.
