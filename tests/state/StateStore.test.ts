...
describe("StateStore runtime metadata", () => {
  ...
  test("atomically attaches an existing remote Session as current", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "agent-bot-state-"));
    tempDirectories.push(directory);
    const store = new StateStore(path.join(directory, "state.sqlite"));
    stores.push(store);
    store.getOrCreateUserContext("chat_id:attached", "codex");

    const attached = store.createSessionAsCurrent({
      localSessionId: "attached",
      contextKey: "chat_id:attached",
      agentName: "codex",
      cwd: directory,
      status: "ready",
      runtimeKind: "codex",
      remoteSessionId: "remote_attached",
      title: "Existing task",
    }, {
      eventType: "session_attached_to_group",
      payload: { remoteSessionId: "remote_attached" },
    });
    ...
    expect(store.getUserContext("chat_id:attached")?.currentSessionId).toBe("attached");

    store.getOrCreateUserContext("chat_id:duplicate", "codex");
    expect(() => store.createSessionAsCurrent({
      localSessionId: "duplicate",
      contextKey: "chat_id:duplicate",
      agentName: "codex",
      cwd: directory,
      status: "ready",
      runtimeKind: "codex",
      remoteSessionId: "remote_attached",
    }, { eventType: "session_attached_to_group", payload: {} })).toThrow();
    expect(store.getSession("duplicate")).toBeUndefined();
    expect(store.getUserContext("chat_id:duplicate")?.currentSessionId).toBeUndefined();
  });
