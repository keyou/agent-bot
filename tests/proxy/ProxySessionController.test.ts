...
describe("ProxySessionController", () => {
  ...
  test("creates a group for an idle existing Session without creating or forking it", async () => {
    const { controller, runtime, remoteSessions, outbound, presenter, store } = fixture();
    remoteSessions.push({
      id: "019f-existing",
    ...
    await controller.onMessage({
      ...message("/newgroup Review room --session codex://threads/019f-existing"),
      userId: "ou_current_user",
    });
  ...
  test.each([
    {
      name: "active",
      remoteId: "019f-active",
      setup: (_store: StateStore): void => undefined,
      remoteStatus: "active" as const,
      expected: "正在外部 Agent 中执行",
    },
    {
      name: "already bound",
      remoteId: "019f-bound",
      setup: (store: StateStore): void => {
        store.createSession({
          localSessionId: "bound",
          contextKey: "chat_id:old",
          agentName: "codex",
          cwd: process.cwd(),
          status: "ready",
        });
        store.updateRuntimeSession("bound", { runtimeKind: "codex", remoteSessionId: "019f-bound" });
      },
      remoteStatus: "idle" as const,
      expected: "不能重复创建群",
    },
  ])("rejects an $name existing Session before creating a group", async ({
    remoteId, setup, remoteStatus, expected,
  }) => {
    const { controller, runtime, remoteSessions, outbound, store } = fixture();
    setup(store);
    remoteSessions.push({
      ...
      source: "codex",
      status: remoteStatus,
      lastTurnStatus: remoteStatus === "active" ? "inProgress" : "completed",
    });

    await controller.onMessage({
      ...message(`/newgroup --session ${remoteId}`),
      userId: "ou_current_user",
    });

    expect(outbound.createGroup).not.toHaveBeenCalled();
    expect(runtime.createSession).not.toHaveBeenCalled();
    expect(outbound.sendText).toHaveBeenCalledWith("chat_id:c1", expect.stringContaining(expected));
  });
  ...
  test("creates a CLI group for an existing Session ID", async () => {
    const { controller, runtime, remoteSessions, store } = fixture();
    ...
    const created = await controller.controlCreateTaskGroup(
      ...
      false,
      undefined,
      "codex://threads/019f-cli-existing",
    );
