...
describe("CommandRouter", () => {
  ...
  test("parses newgroup existing Session mode", () => {
    expect(router.parse('/newgroup "Existing task" --session 019f-thread --agent traex')).toEqual({
      type: "newgroup",
      title: "Existing task",
    ...
    expect(() => router.parse("/newgroup --session 019f-thread --dir ~/project"))
      .toThrow("不能和 --dir 或 --nodir");
